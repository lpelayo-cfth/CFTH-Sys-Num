...
# Set the path to master zipcode file from USPS
master_zip <- "Data/Excel format/ZIP_Locale_Detail.xls"
 
# Read data from Sheet 1
zip_code_data <- read_excel(master_zip, sheet = 1)
 
# Define column names
zip_code_column <- "DELIVERY ZIPCODE"
state_column <- "PHYSICAL STATE"
 
# Ensure unique zip codes in zip_code_data dataframe
zip_code_data_unique <- zip_code_data %>%
  select(!!sym(zip_code_column), !!sym(state_column)) %>%
  distinct()
 
# Merge BNL data with zip_code_data_unique
BNL_zip_code_counts <- by_name_list %>%
  distinct(ClientId, .keep_all = TRUE) %>%
  left_join(zip_code_data_unique, by = c("PriorZipCode" = zip_code_column)) %>%
  mutate(
    State = ifelse(is.na(!!sym(state_column)), "Not Valid", !!sym(state_column)),
    PriorZipCode = ifelse(is.na(PriorZipCode) | PriorZipCode == "", "Unknown", PriorZipCode)
  ) %>%
  group_by(PriorZipCode, State, StatedReasonforHomelessness) %>%  # Retain all three columns
...
# Group by state and count total homeless people per state
state_homeless_summary <- BNL_zip_code_counts %>%
  group_by(State) %>%
  summarise(people_count = sum(people_count, na.rm = TRUE)) %>%
  ungroup() %>%
  arrange(desc(people_count))
 
# Separate the top 24 states
top_24_states <- state_homeless_summary %>%
  slice_head(n = 24)
 
# Group remaining states as "Other States"
other_states <- state_homeless_summary %>%
  slice(-c(1:24)) %>%
  summarise(State = "Other States", people_count = sum(people_count, na.rm = TRUE))
 
# Combine the top 24 states with "Other States"
final_state_summary <- bind_rows(top_24_states, other_states)
 
# Print the result
print(final_state_summary)
 
# Print the result
print(state_homeless_summary)

```


```{r Clark County Analysis}

# Define Clark County ZIP codes
clark_county_zip_codes <- c(
  "98601", "98604", "98606", "98607", "98629", "98642", 
  "98660", "98661", "98662", "98663", "98664", "98665",  
  "98682", "98683", "98684", "98685", "98686", "98671", 
  "98674", "98675"
)
 
# Filter for Clark County ZIP codes
clark_county_summary <- BNL_zip_code_counts %>%
  filter(PriorZipCode %in% clark_county_zip_codes) %>%
  group_by(PriorZipCode) %>%
  summarise(people_count = sum(people_count, na.rm = TRUE)) %>%
  ungroup()
 
# Calculate the total across all Clark County ZIP codes
total_clark_county_people <- sum(clark_county_summary$people_count, na.rm = TRUE)
 
# Add a row for the total
clark_county_summary <- bind_rows(
  clark_county_summary,
  tibble(PriorZipCode = "Total", people_count = total_clark_county_people)
)
 
# Print the results
print(clark_county_summary)

```


```{r PRIMARY REASON ANALYSIS}

# Filter for Clark County ZIP codes and group by reason
clark_county_reasons <- BNL_zip_code_counts %>%
  filter(PriorZipCode %in% clark_county_zip_codes) %>%
  group_by(StatedReasonforHomelessness) %>%
  summarise(people_count = sum(people_count, na.rm = TRUE)) %>%
  ungroup()
 
# Calculate total people in Clark County for percentage calculation
total_clark_county_people <- sum(clark_county_reasons$people_count, na.rm = TRUE)
 
# Add percentage column and sort in descending order
clark_county_reasons <- clark_county_reasons %>%
  mutate(percentage = round((people_count / total_clark_county_people) * 100, 2)) %>%
  arrange(desc(people_count))  # Sort from largest to smallest
 
# Print result
print(clark_county_reasons)
...
portland_zip_codes <- c(
  ...
  "97225", "97227", "97228", "97229", "97230", "97231", "97232", "97233", 
  "97236", "97238", "97239", "97240", "97242", "97258", "97266", "97267", 
  "97268", "97269", "97280", "97281", "97282", "97283", "97286", "97290", 
  "97291", "97292", "97293", "97294", "97296", "97298", "97299"
)

# Filter for Portland ZIP codes and sum people count
total_portland_people <- BNL_zip_code_counts %>%
  filter(PriorZipCode %in% portland_zip_codes) %>%
  summarise(total_people = sum(people_count, na.rm = TRUE)) %>%
  pull(total_people)
 
# Print result
print(total_portland_people)
